...
sync_source_with_repository() {
  ...
  then
    ...
    git clone -b master \
        https://github.com/laplacian-core/model.project-template.git \
        $TARGET_PROJECT_DIR
  else
...
project:
  ...
  description:
    ...
    zh: |
      The model.project-template project.
  source_repository:
    url: https://github.com/laplacian-core/model.project-template.git
    branch: master
  module_repositories:
    local:
      ../../../mvn-repo
...
show_usage () {
cat << 'END'
Usage: ./scripts/generate-model-project-template.sh [OPTION]...
  -h, --help
