...
TARGET_PROJECT_DIR="$PROJECT_BASE_DIR/subprojects/project-project-types"
...
sync_source_with_repository() {
  ...
  then
    ...
    git clone -b master \
        https://github.com/laplacian-core/project.project-types.git \
        $TARGET_PROJECT_DIR
  else
...
project:
  ...
  description:
    ...
    zh: |
      定义了每个项目类型的内容的模型数据。
  source_repository:
    url: https://github.com/laplacian-core/project.project-types.git
    branch: master
  module_repositories:
    local:
      ../../../mvn-repo
...
show_usage () {
cat << 'END'
Usage: ./scripts/generate-project-project-types.sh [OPTION]...
  -h, --help
