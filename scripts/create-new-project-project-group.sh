...
run_create_new_project_project_group() {
  parse_args "$@"
  read_user_input
  ! [ -z $VERBOSE ] && set -x
...
show_usage () {
cat << 'END'
Usage: ./scripts/create-new-project-project-group.sh [OPTION]...
  -h, --help
...
source $SCRIPT_BASE_DIR/.create-new-project-project-group/main.sh
run_create_new_project_project_group "$@"